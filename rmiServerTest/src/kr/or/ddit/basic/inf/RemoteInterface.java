...
// RMI용 interface는 Remote를 상속해서 작성한다.

public interface RemoteInterface extends Remote{
	
	// 이 인터페이스에서 선언되는 모든 메서드들은 RemoteException을 throws 하도록 작성해야 한다.
	
	// 이 인터페이스에서 선언된 메소드의 파라미터 변수는 클라이언트에서 서버쪽으로 보내는 데이터가
	// 저장되는 것이고, 선언된 메소드의 반환값은 서버에서 처리한 결과를 클라이언트로 전달하는 데이터이다. 
