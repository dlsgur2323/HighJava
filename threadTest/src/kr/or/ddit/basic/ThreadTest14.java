 ...
 * 영향을 줄 수 있다. 그래서 stop()메소드는 비추천으로 되어 있다.
 * 
 */
...
// interrupt()메소드를 이용하여 쓰레드를 멈추게 하는 연습용 쓰레드
class ThreadStopTest2 extends Thread {
	...
	public void run() {
		...
		while(true){
			System.out.println("쓰레드 실행.... 중...");
			// interrupt()메소드가 호출되었는지 여부를 검사한다.
			
			// 검사방법1 ==> 인스턴스 메소드인 isInterrupted()메소드를 이용해서 검사한다.
			// 			  isInterrupted() 메소드는 interrupt()메소드가 호출되면 true를 반환한다.
//			if(this.isInterrupted()){
//				break;
//			}
			
			// 검사방법2 ==> Thread의 정적 메소드인 interrupted()메소드를 이용해서 검사한다.
			//			  interrupt()메소드가 호출되면 true를 반환한다.
